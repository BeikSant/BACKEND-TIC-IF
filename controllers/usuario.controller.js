...
usuarioController.login = async (req, res) => {
  ...
  try {
    ...
    if (!user.estado)
      return res
        .status(404)
        .json({ message: "La cuenta se encuentra inactiva" });
...
usuarioController.updatePassword = async (req, res) => {
  ...
  if (!password)
    return res.status(404).json({ message: "No existe la contraseña actual" });
  if (!newpassword)
    return res.status(404).json({ message: "No existe la nueva contraseña" });
  try {
...
usuarioController.generarTokenRecuperacion = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      return res
        .status(404)
        .json({ message: "El correo no pertence a ninguna cuenta registrada" });
    if (!user.estado)
      return res
        .status(404)
        .json({
          message:
            "La cuenta se encuentra inactiva, comuníquese con el Director",
        });
    if (user.tokenRecuperacion && user.tokenRecuperacion != null) {
      if (new Date() < new Date(user.tokenExpire))
        return res
          .status(200)
          .json({ message: "El email ya ha sido enviado a su correo" });
...
usuarioController.cambiarEstado = async (req, res) => {
  ...
  try {
    ...
    if (director == docente._id)
      return res
        .status(404)
        .json({ message: "No puede cambiar el estado de su propia cuenta" });
    ...
    if (estado)
      return res
        .status(200)
        .json({ message: "Se ha inactivado la cuenta del docente con éxito" });
    return res
      .status(200)
...
usuarioController.cambiarRol = async (req, res) => {
  ...
  if (director == iddocente)
    return res
      .status(404)
      .json({ message: "No puede cambiar el rol de su propia cuenta" });
  ...
  return res
    .status(200)
    .json({ message: "Se ha cambiado el rol del docente con éxito" });
};
