...
formatoController.crear = async (req, res) => {
  if (!req.body.formato)
    return res
      .status(404)
      .json({ message: "No existen un formato para registrar" });
  const formato = {
    ...
    actividadesDistributivo:
      req.body.formato.actividadesDistributivo.toString(),
    horasPAO: req.body.formato.horasPAO.toString(),
    actividadesDesarrolladas:
      req.body.formato.actividadesDesarrolladas.toString(),
  ...
  if (!formatoNuevo)
    return res
      .status(404)
      .json({ message: "Ocurrio un error al crear el formato" });
  return res
    .status(200)
    .json({ message: "El formato del informe final se ha creado con éxito" });
};

formatoController.obtenerTodos = async (req, res) => {
  let formatos = await formatoModel.find().lean();
  if (!formatos)
    return res.status(404).json({ message: "No existen formatos registrados" });
  return res.status(200).json({ formatos });
};

formatoController.descargar = async (req, res) => {
  if (!mongoose.isValidObjectId(req.params.id))
    return res.status(404).json({ message: "Formato no encontrado" });
  const formato = await formatoModel.findById(req.params.id).lean();
  if (!formato)
    return res.status(404).json({ message: "Formato no encontrado" });
  const pdf = await generarInformePDF({ formato }, true);
...
formatoController.actualizar = async (req, res) => {
  ...
  if (!formato)
    return res.status(404).json({ message: "No se encontró el formato" });
  if (formato.tipo == "POR DEFECTO")
    return res.status(403).json({ message: "No se puede editar este formato" });
  await formato.updateOne(req.body.formato);
  return res
    .status(200)
    .json({ message: "El formato se ha actualizó con éxito" });
};
...
formatoController.cambiarEstado = async (req, res) => {
  ...
  if (!formatoNuevo)
    return res.status(404).json({ message: "No se pudo encontrar el formato" });
  await formatoNuevo.updateOne({ estado: true });
  return res
    .status(200)
    .json({
      message: "Se ha actualizado con éxito el formato del informe final",
    });
};
...
formatoController.obtenerActivo = async (req, res) => {
  try {
    ...
    if (!formato)
      return res
        .status(404)
        .json({ message: "No existe un formato activo o disponible" });
    return res.status(200).json(formato);
  } catch (error) {
    console.error(error);
...
formatoController.eliminar = async (req, res) => {
  ...
  if (formato.estado)
    return res
      .status(404)
      .json({ message: "Este formato no se puede eliminar" });
  if (formato.tipo == "POR DEFECTO")
    return res
      .status(403)
      .json({ message: "No se puede eliminar este formato" });
