...
docenteController.obtenerTodos = async (req, res) => {
  ...
  let docentes = await docenteModel
    .find(filter)
    .populate([
      "carrera",
      {
...
docenteController.obtenerUno = async (req, res) => {
  ...
  try {
    const docente = await docenteModel.findById(req.user.docente).populate([
      {
        path: "usuario",
        populate: {
          path: "rol",
...
docenteController.crear = async (req, res) => {
  try {
    ...
    if ((await docenteModel.find({ correo: docente.correo })).length > 0)
      return res
        .status(404)
        .json({ message: "El correo ya pertenece a otro docente" });
    const iddirector = req.user.docente;
    //Obtiendo la carrera del director
    const director = await docenteModel.findById(iddirector).populate('carrera');
    //Asignando la carrera del director al docente
    docente.carrera = director.carrera._id;
    ...
    if (!usuario) {
      ...
      return res
        .status(404)
        .json({ message: "Ocurrió un error al crear al docente" });
    }
    ...
    const data = {
      docente: {
        ...
        carrera: director.carrera.nombre
      },
      email: doc.correo,
      password: doc.primerNombre + "." + doc.primerApellido + "@" + "Docente",
      enlace: process.env.ORIGIN_1
    };
    await mail.nuevaCuenta(data);
    return res
      .status(200)
      .json({ message: "Docente creado con éxito", docente: doc });
  } catch (error) {
    console.error(error);
    return res
      .status(404)
...
docenteController.editar = async (req, res) => {
  ...
  if (!mongoose.isValidObjectId(idDocente))
    return res.status(404).json({ message: "No se encontró al docente" });
  ...
  try {
    ...
    const docenteUpdt = await docenteModel.findById(idDocente).populate({
      path: "usuario",
      populate: {
        path: "rol",
      },
    ...
    return res
      .status(200)
      .json({
        message: "Información del docente actualizada con éxito",
        docente: docenteUpdt,
      });
  } catch (error) {
    console.error(error);
...
docenteController.editarDedicacion = async (req, res) => {
  ...
  if (!dedicacion)
    return res
      .status(404)
      .json({
        message: "No existe la dedicación, comuníquese con el administrador",
      });
  try {
    const docenteFind = await docenteModel.findById(idDocente);
    if (!docenteFind)
      return res
        ...
        .json({
          message:
            "No se encontró al docente, comuníquese con el administrador",
        });
    await docenteFind.updateOne({ dedicacion });
    return res
      .status(200)
      .json({ message: "Se actualizó con éxito su dedicación" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Error interno del servidor" });
  }
